...
class Cov1dBlock(nn.Module):
    ...
    def forward(self, xs, hid=None):
        ...
        # if self.drop_out_prob != -1:
        #     output = F.dropout(output, self.drop_out_prob, training=True)
...
class WaveToLetter(nn.Module):
    def __init__(self,sample_rate,window_size, labels="abc",audio_conf=None,mixed_precision=False):
        ...
        conv1 =  Cov1dBlock(input_size=13,output_size=256,kernal_size=(11,),stride=2,dilation=1,drop_out_prob=0.2,padding='same')
        conv2s = []
        ...
        for idx in range(15):
            layergroup = idx//3
            if (layergroup) == 0:
                convTemp = Cov1dBlock(input_size=inputSize,output_size=256,kernal_size=(11,),stride=1,dilation=1,drop_out_prob=0.2,padding='same')#11
                conv2s.append(('conv1d_{}'.format(idx+1),convTemp))#change id
                inputSize = 256
            elif (layergroup) == 1:
                convTemp = Cov1dBlock(input_size=inputSize, output_size=384, kernal_size=(13,), stride=1, dilation=1,#13
                                      drop_out_prob=0.2)
                conv2s.append(('conv1d_{}'.format(idx+1), convTemp))
                inputSize=384
            elif (layergroup) ==2:
                convTemp = Cov1dBlock(input_size=inputSize, output_size=512, kernal_size=(17,), stride=1, dilation=1,#17
                                      drop_out_prob=0.2)
                conv2s.append(('conv1d_{}'.format(idx+1), convTemp))
                inputSize = 512

            elif (layergroup) ==3:
                convTemp = Cov1dBlock(input_size=inputSize, output_size=640, kernal_size=(21,), stride=1, dilation=1,#21
                                      drop_out_prob=0.3)
                conv2s.append(('conv1d_{}'.format(idx+1), convTemp))
                inputSize = 640

            elif (layergroup) ==4:
                convTemp = Cov1dBlock(input_size=inputSize, output_size=768, kernal_size=(25,), stride=1, dilation=1,#25
                                      drop_out_prob=0.3)
                conv2s.append(('conv1d_{}'.format(idx+1), convTemp))
                inputSize = 768
    ...
    def load_model(cls, path, cuda=False):
        ...
        model = cls(labels=package['labels'], audio_conf=package['audio_conf'],sample_rate=16000
    ...
    def load_model_package(cls, package, cuda=False):
        ...
        model.load_state_dict(package['state_dict'])
        best_wer = package['best_wer'] if 'best_wer' in package.keys() else 0. #change
        if cuda:
    ...
    def serialize(model, optimizer=None, epoch=None, iteration=None, loss_results=None,
                  cer_results=None, wer_results=None, avg_loss=None, meta=None, best_wer=None): ##change
        #model = model.module # DataParallel
