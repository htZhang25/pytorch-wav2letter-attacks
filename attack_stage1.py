## attack.py -- the first stage for generating audio adversarial examples
##
## 
...
parser.add_argument('--max-iterations', default=50000, type=int, help='maximum number of iteration for attacks')
parser.add_argument('--target', default=None, type=str, help='target sentence for attacks')

parser.add_argument('--audio-path', default='./data/original/original1.wav', type=str, help='input audio path for attacks')
parser.add_argument('--orig-path', default='./data/original/original1.wav', type=str, help='original audio path for attacks')
parser.add_argument('--target-path', default='./data/target/target1.txt', type=str, help='original txt path for attacks')
...
parser.add_argument('--lr', '--learning-rate to attack', default=8e-6, type=float, help='initial learning rate')
parser.add_argument('--learning-anneal', default=10, type=float, help="Annealing applied to learning rate every epoch")
...
class Attacker(object):
    ...
    def save(self, signal, target_path, orig_path, lr, iterations, i, bandwidth):
        outprefix = "generated_stage1"
        if not os.path.exists(outprefix):
            os.makedirs(outprefix)
        signal = signal.unsqueeze(0)
        id_original = self.audio_path.split("/")[-1].split('.')[0]
        id_target = target_path.split("/")[-1].split('.')[0]
        save_path = os.path.join(outprefix, id_original + '_to_' + id_target + "_stage1" + ".wav")
    ...
    def save_figure(self, audio, save_path):
        ...
        audio = audio.data.numpy() / np.max(audio.data.numpy())
...
if __name__ == '__main__':
    ...
    transcript_origin = args.orig_path[:-4] + ".txt"
    print(transcript_origin)
    fp_a=open(transcript_origin)
    transcript_a=fp_a.readlines()[0]
    print(transcript_a.lower())
    print('\n')

    transcript_target = args.target_path
    print(transcript_target)
    fp_b=open(transcript_target)
    transcript_b=fp_b.readlines()[0]
    print(transcript_b.lower())
    print('\n')

    attacker = Attacker(audio_path=args.orig_path, orig_path=args.orig_path, audio_conf=audio_conf, labels=labels)
