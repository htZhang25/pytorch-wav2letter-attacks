...
def load_audio(path):
    sound, _ = torchaudio.load(path, normalization=True)#, normalization=True
    if len(sound.shape) > 1:
        # print("sound.shape[0]", sound.shape[0])
        if sound.shape[0] == 1:            
            sound = sound.squeeze()
            # print("sound", sound[0:20])
        else:
...
class SpectrogramDataset(Dataset, SpectrogramParser):
    ...
    def __getitem__(self, index):
        ...
        fp=open(transcriptLoaded)

        transcriptToUse=fp.readlines()[0]
        if self.w2l2:
            spect = self.parse_audio_w2l2(audio_path)
        ...
        return spect, transcript, audio_path, transcriptToUse
...
def _collate_fn(batch):
    ...
    for x in range(minibatch_size):
        ...
        target = sample[1]
        tensorPath = sample[2]
        orignalTranscription = sample[3]
        seq_length = tensor.shape[1]
        tensorShape = tensor.shape
        pad_dims = (0,abs(tensorShape[1]-max_seqlength), 0, 0)
        tensorNew = F.pad(tensor, pad_dims,"constant")
    ...
    return inputs, targets, input_percentages, target_sizes, inputFilePathAndTranscription
